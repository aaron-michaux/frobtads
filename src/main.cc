...
const char versionOutput[]
    = "FrobTADS " PACKAGE_VERSION "\n"
      ...
      "You may redistribute copies of FrobTADS under certain terms and "
      "conditions.\n"
...
const char helpOutput[]
    = "Options:\n"
      ...
      "  -f, --force-colors   Try to enable colors even if the system claims "
      "that\n"
      ...
      "  -e, --scroll-buffer  Size of the scroll-back buffer. Must be between "
      "8 and\n"
      "                       8192kB (default is 512kB)\n"
      "  -r, --restore        Load a saved game position\n"
      "  -R, --replay         Replay commands from the specified file\n"
      "  -u, --undo-size      Multiply the availabe T3VM undo buffer by n. "
      "Must be\n"
      "                       between 1 and 64 (default is 16; about 100 "
      "UNDOs)\n"
      "  -k, --character-set  Use given charset as the keyboard and display\n"
      "                       character set.\n"
      "  -i, --interface      Use given screen interface (curses or plain). "
      "Default\n"
      "                       is curses. (Advanced features like statusline, "
      "banners\n"
      "                       and colors are not available when using the "
      "plain\n"
      "                       interface.)\n"
      "Color codes:\n"
      ...
      "\n"
      "Short options are case-sensitive, long options are not. Options may be "
      "given\n"
      "in any order and may be specified both before and after the filename. "
      "If an\n"
      "option that takes an argument is given multiple times the last value is "
      "used.\n"
      "Options may be abbreviated: --no-scr will be recognized as "
      "--no-scrolling.\n"
      "\n"
...
#ifdef TADSNET
static void create_netconfig(TadsNetConfig*& c, const char* const* argv)
{
...
int main(int argc, char** argv)
{
   ...
   while(filename == 0 && (optChar = opts(iter, optArg)) != Options::ENDOPTS)
      switch(optChar) {
      // --version
      case 'v': cout << versionOutput << "\n\n"; return 0;
      ...
      // --no-colors
      case 'n': frobOpts.useColors = false; break;

      // --force-colors
      case 'f': frobOpts.forceColors = true; break;

      // --no-defcolors
      case 'o': frobOpts.defColors = false; break;

      // --no-scrolling
      case 'c': frobOpts.softScroll = false; break;

      // --no-pause
      case 'p': frobOpts.exitPause = false; break;

      // --no-chdir
      case 'd': frobOpts.changeDir = false; break;
      ...
      case 'u': {
         ...
         if(tmp < 1 or tmp > 64) {
            // Out of range.
            cerr << opts.name()
                 << ": undo multiplicator must be between 1-64.\n";
            optionError = true;
            break;
         }
      ...
      case Options::BADKWD: optionError = true; break;

      case Options::AMBIGUOUS: optionError = true; break;
      }
   ...
   if(filename == 0 and not optionError) {
      if(savedPosFilename != 0) {
         ...
         int engtyp;
         if(vm_get_game_arg(
                3, argvDummy, detectedFilename, OSFNMAX + 1, &engtyp)) {
   ...
   case VM_GGT_TADS2:
      ...
      case cursesInterface:
         return FrobTadsApplicationCurses(frobOpts).runTads(actualFilename, 0);
      case plainInterface:
         return FrobTadsApplicationPlain(frobOpts).runTads(actualFilename, 0);
      }
   ...
   case VM_GGT_TADS3: {
      ...
      switch(interface) {
      case cursesInterface:
         t3vmRet = FrobTadsApplicationCurses(frobOpts).runTads(actualFilename,
                                                               1,
                                                               progArgc,
                                                               progArgv,
                                                               savedPosFilename,
         ...
         break;
      case plainInterface:
         t3vmRet = FrobTadsApplicationPlain(frobOpts).runTads(actualFilename,
                                                              1,
                                                              progArgc,
                                                              progArgv,
                                                              savedPosFilename,
