...
class FrobTadsApplication
{
 ...
 private:
   /* Runs the T2VM.
    */
   int fRunTads2(char* filename);

   /* Runs the T3VM.
    */
   int fRunTads3(char* filename,
                 int argc,
 ...
 public:
    ...
    */
   virtual void moveCursor(int line, int column) = 0;

   /* Prints a string at the specified position, using 'attrs' as
    * atributes.
    */
   virtual void print(int line, int column, int attrs, const char* str) = 0;
    ...
    */
   virtual void clear(int top, int left, int bottom, int right, int attrs) = 0;
    ...
    */
   virtual int getRawChar(bool cursorVisible, int timeout) = 0;
