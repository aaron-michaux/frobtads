...
/* We need a signal handler to handle window resizes when running
 * inside a terminal.
 */
void winResizeHandler(int)
{
...
int FrobTadsApplication::fRunTads2(char* filename)
{
...
int FrobTadsApplication::fRunTads3(char* filename,
                                   int argc,
                                   ...
                                   class TadsNetConfig* netconfig)
{
   ...
   hostifc->set_io_safety(this->options.safetyLevelR,
                          this->options.safetyLevelW);

   // Set the network safety level.
   hostifc->set_net_safety(this->options.netSafetyLevelC,
                           this->options.netSafetyLevelS);
...
int FrobTadsApplication::runTads(const char* filename,
                                 int vm,
                                 int argc,
                                 const char* const* argv,
                                 const char* savedState,
                                 class TadsNetConfig* netconfig)
{
   ...
   // wrong colors.
   int tmpColor
       = ossgetcolor(OSGEN_COLOR_STATUSLINE, OSGEN_COLOR_STATUSBG, 0, 0);
   ossdsp(0, 0, tmpColor, " ");
   globalApp->flush();
   ...
   // Run the VM.
   int vmRet = vm == 0
                   ? this->fRunTads2(finalFilenamePtr)
                   : this->fRunTads3(
                         finalFilenamePtr, argc, argv, savedState, netconfig);
...
#include <windows.h>
#endif
bool FrobTadsApplication::changeDirectory(const char* dir)
{
#ifdef HAVE_CHDIR
