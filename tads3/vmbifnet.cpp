...
{
   fprintf(stderr, "Web UI disable, sorry!\n");
   ...
   if(path == 0) err_throw(VMERR_BAD_TYPE_BIF);
    ...
    */
   if(G_net_config == 0) G_net_config = new TadsNetConfig();
   ...
   if(ok) {
      ...
      errmsg = lib_copy_str("No address information available for HTTPServer");
   }
   ...
   if(!ok) {
      ...
      G_interpreter->throw_new_class(
          vmg_ G_predef->net_exception, 1, "Error connecting to Web UI");
   }
...
void CVmBifNet::get_event(VMG_ uint oargc)
{
   ...
   err_try
   {
      ...
      evt_error:
         ...
         G_interpreter->throw_new_class(vmg_ G_predef->net_exception, 1, err);
       ...
       */
      if(ev_cl == VM_INVALID_OBJ) ev_cl = G_predef->net_event;
...
void CVmBifNet::get_storage_url(VMG_ uint oargc)
{
   ...
   if(page == 0) err_throw(VMERR_STRING_VAL_REQD);
   ...
   if(host != 0) {
      ...
      G_interpreter->push_stringf(
          vmg_ "http://%s%s%.*s", host, rootpath, (int) pagelen, page);
...
void CVmBifNet::get_launch_host_addr(VMG_ uint oargc)
{
   ...
   const char* host = (G_net_config != 0 ? G_net_config->get("hostname") : 0);
...
{
 public:
   TadsHttpReqResult(VMG_ vm_globalvar_t* idg,
                     int status,
                     CVmDataSource* reply,
                     char* hdrs,
                     char* loc)
   ...
   virtual vm_obj_id_t prep_event_obj(VMG_ int* argc, int* evt_type)
   {
      ...
      if(reply != 0) {
         ...
         vm_obj_id_t fileobj = CVmObjFile::create(
             vmg_ FALSE, 0, csobj, reply, mode, VMOBJFILE_ACCESS_READ, TRUE);
...
class HttpReqThread : public OS_Thread
{
 public:
    ...
    */
   HttpReqThread(VMG_ const vm_val_t* id,
                 const char* url,
                 const char* verb,
                 int32_t options,
                 const char* hdrs,
                 ...
                 const vm_rcdesc* rc)
   {
      ...
      if((queue = G_net_queue) != 0) queue->add_ref();
      ...
      size_t pfx = (urll >= 7 && memcmp(url, "http://", 7) == 0
                        ? 7
      ...
      const char* start;
      for(start = url; urll > 0 && *url != ':' && *url != '/'; ++url, --urll)
         ;
      ...
      if(urll > 0 && *url == ':') {
         ...
         for(--urll, start = ++url, port = 0; urll > 0 && *url != '/';
             ++url, --urll) {
            if(is_digit(*url)) port = port * 10 + value_of_digit(*url);
         }
      }
      ...
      this->hdrs
          = (hdrs == 0 ? 0 : lib_copy_str(hdrs + VMB_LEN, vmb_get_len(hdrs)));
      ...
      if(body != 0) {
         ...
         if(body->typ == VM_OBJ
            ...
            size_t mime_type_len  = (mime_type != 0 ? strlen(mime_type) : 0);
            ...
            /* add the stream */
            this->body->add("", 0, "", 0, mime_type, mime_type_len, s);
         }
      }
   }
   ...
   ~HttpReqThread()
   {
      ...
      if(queue != 0) queue->release_ref();

      /* delete our ID global, if we still own it */
      if(idg != 0) G_obj_table->delete_global_var(idg);
   }
   ...
   void post_result(int status, CVmDataSource* reply, char* hdrs, char* loc)
   {
      ...
      TadsHttpReqResult* evt
          = new TadsHttpReqResult(vmg_ idg, status, reply, hdrs, loc);
 ...
 protected:
                                   ...
                                   const char** def_mime_type)
   {
      ...
      if(bstr != 0) {
         ...
         *def_mime_type = "text/plain; charset=utf-8";
         ...
         return new CVmPrivateStringSource(bstr + VMB_LEN, vmb_get_len(bstr));
         ...
         *def_mime_type = "application/octet-stream";
         ...
         CVmObjByteArray* barr = (CVmObjByteArray*) vm_objp(vmg_ val->val.obj);
    ...
    */
   static void
   iter_body_table(VMG_ const vm_val_t* key, const vm_val_t* val, void* ctx0)
   {
      /* get the context */
      ...
      if(val->is_instance_of(vmg_ G_predef->file_upload_cl)) {
         ...
         G_interpreter->get_prop(
             vmg_ & file_val, val, G_predef->file_upload_fileObj, 0, ctx->rc);
         G_interpreter->get_prop(vmg_ & ctype_val,
                                 val,
                                 G_predef->file_upload_contentType,
                                 0,
                                 ctx->rc);
         G_interpreter->get_prop(
             vmg_ & fname_val, val, G_predef->file_upload_filename, 0, ctx->rc);
         ...
         /* add the field */
         ctx->self->body->add(field_name + VMB_LEN,
                              vmb_get_len(field_name),
                              fname,
                              fname_len,
                              ctype,
                              ctype_len,
...
static int match_url_domain(const char* url, size_t urll, const char* domain)
{
   ...
   for(; urll > 0; ++url, --urll) {
      ...
      if(urll > 3 && memcmp(url, "://", 3) == 0) {
          ...
          */
         return (urll >= domainl && memicmp(url, domain, domainl) == 0
                 && (urll == domainl || url[domainl] == ':'
                     || url[domainl] == '/'));
      }
   }
...
void CVmBifNet::send_net_request(VMG_ uint argc)
{
    ...
    */
   vm_rcdesc rc(
       vmg_ "sendNetRequest", CVmBifNet::bif_table, 6, G_stk->get(0), argc);
   ...
   if(client_level >= VM_NET_SAFETY_MAXIMUM) err_throw(VMERR_NETWORK_SAFETY);
   ...
   /* log the request */
   vm_log_fmt(
       vmg_ "client request to %.*s by %s", (int) lurll, lurl, image_url);
