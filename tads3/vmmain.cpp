...
int vm_run_image(const vm_run_image_params* params)
{
    ...
    */
   os_get_abs_filename(
       G_os_gamename, sizeof(G_os_gamename), params->image_file_name);
   ...
   /* initialize the VM */
   vm_init_options opts(params->hostifc,
                        params->clientifc,
                        params->charset,
                        params->log_charset);
   ...
   err_try
   {
      ...
      if(params->res_dir != 0) params->hostifc->set_res_dir(params->res_dir);
      ...
      params->clientifc->client_init(VMGLOB_ADDR,
                                     params->script_file,
                                     params->script_quiet,
                                     params->log_file,
                                     params->cmd_log_file,
                                     params->show_banner ? T3VM_BANNER_STRING
                                                         : 0,
                                     params->more_mode);
      ...
      if(params->load_from_exe) {
         ...
         if(exe_fp == 0) err_throw(VMERR_NO_IMAGE_IN_EXE);
      ...
      if(params->seed_rand) CVmBifTADS::randomize(vmg_ 0);
      ...
      loader->run(
          vmg_ params->prog_argv, params->prog_argc, 0, 0, params->saved_state);
    ...
    */
   if(G_net_config != 0) delete G_net_config;
#endif
 ...
 */
#ifdef TADSNET
static void create_netconfig(CVmMainClientIfc* clientifc,
                             ...
                             char** argv)
{
   if(netconfig == 0) {
      ...
      os_get_special_path(
          confpath, sizeof(confpath), argv[0], OS_GSP_T3_SYSCONFIG);
      os_build_full_path(
          confname, sizeof(confname), confpath, "tadsweb.config");
      ...
      if(fp != 0) {
         ...
         char* msg
             = t3sprintf_alloc("Warning: -webhost mode specified, but couldn't "
                               "read web configuration file \"%s\"",
                               confname);
                      ...
                      CVmHostIfc* hostifc)
{
   ...
   /* check to see if we can load from the .exe file */
   {
       ...
       *   if defined
       */
      if(params.load_from_exe && (fp = os_exeseek(argv[0], "SAVX")) != 0) {
         ushort len;
         char buf[OSFNMAX];

         /* read the length and the name */
         if(!osfrb(fp, &len, sizeof(len)) && len < sizeof(buf)
            && !osfrb(fp, buf, len)) {
   ...
   for(curarg = 1; curarg < argc && argv[curarg][0] == '-'; ++curarg) {
      ...
      case 'n':
         if(argv[curarg][2] == 's') {
            ...
            if(*cli == '\0' || *srv == '\0') srv = cli;

            /* validate the arguments */
            if(*cli < '0' || *cli > '2' || *srv < '0' || *srv > '2')
               goto opt_error;
      ...
      case 's':
         if(argv[curarg][2] == 'd') {
            ...
            if(*rd == '\0' || *wrt == '\0') wrt = rd;

            /* check the ranges */
            if(*rd < '0' || *rd > '4' || *wrt < '0' || *wrt > '4'
               || *(wrt + 1) != '\0') {
               /* invalid level */
               goto opt_error;
            } else {
      ...
      case 'w':
         ...
         } else if(strcmp(argv[curarg], "-websid") == 0 && curarg + 1 < argc) {
   ...
   if(usage_err) {
   ...
   } else if(!params.load_from_exe && curarg + 1 <= argc
      ...
      if(params.load_from_exe
         && os_get_exe_filename(
                image_file_name, sizeof(image_file_name), argv[0]))
         found_image = TRUE;
      ...
      if(!params.load_from_exe && !found_image && params.saved_state != 0) {
         ...
         if(save_fp != 0) {
            ...
            if(CVmSaveFile::restore_get_image(
                   save_fp, image_file_name, sizeof(image_file_name))
               == 0) {
   ...
   if(usage_err || !found_image) {
      ...
      if(params.load_from_exe && !usage_err) {
         sprintf(buf,
                 "An error occurred loading the T3 VM program from "
...
static void strcpy_limit(char* dst, const char* src, size_t limit)
{
    ...
    */
   if(copy_len > limit - 1) copy_len = limit - 1;
                                          ...
                                          size_t fnamelen)
{
   ...
   if(!osfrb(fp, buf, 16)) {
      ...
      if(memcmp(buf, "TADS2 save/g\012\015\032\000", 16) == 0) {
         ...
         if(len > fnamelen - 1) len = fnamelen - 1;
   ...
   *engine_type = VM_GGT_INVALID;
   ...
   for(i = 1; i < argc && argv[i][0] == '-'; ++i) {
      ...
      case 'c':
          ...
          *   "-ctab tab"
          */
         if(strcmp(argv[i], "-ctab") == 0 || strcmp(argv[i], "-cs") == 0
            || strcmp(argv[i], "-csl") == 0) {
...
static int vm_get_game_type_for_file(const char* filename)
{
   ...
   if(fp == 0) return VM_GGT_NOT_FOUND;
...
#ifdef WASMTADS
int vm_get_game_type(FILE* fp)
{
   constexpr int k_buf_sz = 16;
   char buf[k_buf_sz];
   int ret = VM_GGT_INVALID;

   if(fp == nullptr) return VM_GGT_NOT_FOUND; // File doesn't exist

   // Preserve the initial file position
   const auto initial_pos = ftell(fp);

   if(osfrb(fp, &buf[0], k_buf_sz) != 0)
      ret = VM_GGT_INVALID; // File read error imples invalid game
   else if(memcmp(buf, "TADS2 bin\012\015\032", 12) == 0)
      ret = VM_GGT_TADS2;
   else if(memcmp(buf, "T3-image\015\012\032", 11) == 0)
      ret = VM_GGT_TADS3;
   else
      ret = VM_GGT_INVALID; // invalid signature

   // Reset the file position
   const bool seek_success = (fseek(fp, initial_pos, SEEK_SET) == 0);
   if(!seek_success) {
      fprintf(stderr,
              "Fatal error: could not reset file position to %d\n",
              int(initial_pos));
      exit(1);
   }
...
int vm_get_game_type(const char* filename,
                     char* actual_fname,
                     size_t actual_fname_len,
                     const char* const* defexts,
                     size_t defext_count)
{
   int good_count;
   int last_good_ver;
    ...
    */
   if(good_count == 1) return last_good_ver;

   /*
    *   If we didn't find any matches, tell the caller there is no match
    *   for the given filename.
    */
   if(good_count == 0) return VM_GGT_NOT_FOUND;
...
void vm_interface_report(CVmMainClientIfc* cli, const char* fname)
{
   ...
   for(i = 0; G_meta_reg_table[i].meta != 0; ++i) {
      ...
      lib_strcpy(nm, sizeof(nm), (*G_meta_reg_table[i].meta)->get_meta_name());
