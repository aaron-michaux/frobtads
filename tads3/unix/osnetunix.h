...
class OS_Event : public CVmRefCntObj, public OS_Waitable
{
 ...
 public:
   ...
   void signal()
   {
       ...
       */
      for(osu_waitsub* s = sub_head; s != 0; s = s->nxt) s->e->signal();
 ...
 private:
   ...
   void evt_wait()
   {
      ...
      while(cnt <= 0) {
         ...
         if(pthread_cond_wait(&cond, &mutex) && errno != EINTR) break;
      }

      /* if it's an auto-reset event, reduce the counter */
      if(!manual_reset) cnt -= 1;
   ...
   int evt_wait(const struct timespec* tm)
   {
      ...
      return (rc == 0 ? OSWAIT_EVENT
                      : rc == ETIMEDOUT ? OSWAIT_TIMEOUT : OSWAIT_ERROR);
   }
   ...
   int evt_test()
   {
      ...
      if(signaled && !manual_reset) cnt -= 1;
