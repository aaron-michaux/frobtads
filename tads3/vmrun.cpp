#ifdef RCSID
static char RCSid[] = "$Header: d:/cvsroot/tads/tads3/VMRUN.CPP,v 1.4 "
                      "1999/07/11 00:46:58 MJRoberts Exp $";
#endif
...
void CVmRun::init()
{
   ...
   prof_stack_max_ = 250;
   (prof_stack_
    = (vm_profiler_rec*) t3malloc(prof_stack_max_ * sizeof(prof_stack_[0])));
...
check_type:
   ...
   case VM_LIST:
      ...
      CVmObjList::add_to_list(
          vmg_ val1, VM_INVALID_OBJ, get_const_ptr(vmg_ val1->val.ofs), val2);
      return TRUE;
   ...
   case VM_OBJ:
       ...
       */
      return vm_objp(vmg_ val1->val.obj)
          ->add_val(vmg_ val1, val1->val.obj, val2);
...
const uchar* CVmRun::compute_sum_inc(VMG_ const uchar* p)
{
   ...
   if(compute_sum(vmg_ get(0), &val2)) {
      ...
      pop(&val);
      push(&val2);
      return op_overload(vmg_ p - entry_ptr_native_,
                         -1,
                         &val,
                         G_predef->operator_add,
                         1,
                         VMERR_BAD_TYPE_ADD);
   }
}
...
check_type:
   ...
   case VM_LIST:
      ...
      CVmObjList::sub_from_list(
          vmg_ val1, val1, get_const_ptr(vmg_ val1->val.ofs), val2);
      return TRUE;

   case VM_OBJ:
      /* object - use the object's virtual subtraction method */
      return vm_objp(vmg_ val1->val.obj)
          ->sub_val(vmg_ val1, val1->val.obj, val2);
...
check_type:
   ...
   case VM_OBJ:
      ...
      return vm_objp(vmg_ val1->val.obj)
          ->mul_val(vmg_ val1, val1->val.obj, val2);
...
check_type:
   ...
   case VM_OBJ:
      ...
      return vm_objp(vmg_ val1->val.obj)
          ->div_val(vmg_ val1, val1->val.obj, val2);
      break;
                      ...
                      const vm_val_t* new_val)
{
   ...
   case VM_OBJ:
      ...
      return vm_objp(vmg_ container_val->val.obj)
          ->set_index_val_q(
              vmg_ container_val, container_val->val.obj, index_val, new_val);
                                      ...
                                      int is_transient)
{
   ...
   if(obj != VM_INVALID_OBJ) {
      ...
      if(is_transient) G_obj_table->set_obj_transient(obj);
   } else {
      /* failed - return nil */
      r0_.set_nil();
   }
...
void CVmRun::makelstpar(VMG0_)
{
   ...
   int lstcnt;
   if(!val.is_listlike(vmg0_) || (lstcnt = val.ll_length(vmg0_)) < 0) {
   ...
   if(!check_space(lstcnt + hdr_depth)) err_throw(VMERR_STACK_OVERFLOW);
                        ...
                        const vm_val_t* index_val)
{
   ...
   case VM_LIST:
      ...
      CVmObjList::index_list(
          vmg_ result, get_const_ptr(vmg_ container_val->val.ofs), index_val);
      return TRUE;
...
class vmrun_prop_eval
{
 public:
   ...
   const uchar*
   get_prop(VMG_ uint caller_ofs, vm_obj_id_t self, vm_prop_id_t target_prop)
   {
   ...
   const uchar* get_prop(VMG_ uint caller_ofs, vm_prop_id_t target_prop)
   {
      this->caller_ofs  = caller_ofs;
      this->argc        = 0;
      this->target_prop = target_prop;
   ...
   const uchar*
   get_prop(VMG_ uint caller_ofs, vm_prop_id_t target_prop, uint argc)
   {
      this->caller_ofs  = caller_ofs;
      this->argc        = argc;
      this->target_prop = target_prop;
   ...
   const uchar* get_prop(VMG0_)
   {
      ...
      if(get_prop_no_eval(vmg0_)) return eval_prop_val(vmg0_);
      ...
      if(G_predef->prop_not_defined_prop != VM_INVALID_PROP) {
         ...
         if(get_prop_no_eval(vmg0_)) {
            ...
            if(val.typ == VM_CODEOFS || val.typ == VM_OBJX
               || val.typ == VM_BIFPTRX) {
               /*
   ...
   int get_prop_no_eval(VMG0_)
   {
      ...
      int (*f_const_get_prop)(VMG_ vm_val_t*,
                              const vm_val_t*,
                              const char*,
                              vm_prop_id_t,
                              vm_obj_id_t*,
      ...
      case VM_OBJ:
         ...
         return vm_objp(vmg_ self.val.obj)
             ->get_prop(
                 vmg_ target_prop, &val, self.val.obj, &defining_obj, &argc);
      ...
      const_common:
         ...
         found = f_const_get_prop(
             vmg_ & val, &self, target_ptr, target_prop, &defining_obj, &argc);
   ...
   const uchar* eval_prop_val(VMG0_)
   {
      ...
      default:
         ...
         if(argc != 0) err_throw(VMERR_WRONG_NUM_OF_ARGS);
      ...
      case VM_OBJX:
         ...
         if(vm_objp(vmg_ val.val.obj)->get_invoker(vmg_ 0)) {
            ...
            return G_interpreter->disp_string_val(vmg_ caller_ofs,
                                                  self.val.obj);
         }
...
#define is_valid_for_jst(v) ((v)->typ == VM_NIL || (v)->typ == VM_INT)
#else
#define false_for_cond(v) \
   ((v)->typ == VM_NIL || ((v)->is_numeric(vmg0_) && (v)->num_is_zero(vmg0_)))
#define true_for_cond(v)                       \
   ((v)->typ == VM_TRUE || (v)->typ == VM_ENUM \
    || ((v)->is_numeric(vmg0_) && !(v)->num_is_zero(vmg0_)))
#define is_valid_for_jst(v) ((v)->typ == VM_NIL || (v)->is_numeric(vmg0_))
#endif
...
void CVmRun::run(VMG_ const uchar* start_pc)
{
    ...
    *
    *   NOTE: removing 'register' declaration, because as of C++17, it is
    *         non-conforming c++. -- AJM
    */
...
resume_execution:
   ...
   err_try
   {
      ...
      for(;;) {
         ...
         if(halt_vm_) err_throw(VMERR_DBG_HALT);
      ...
      exec_instruction:
          ...
          */
         switch(*p++) {
         case OPC_GETARGN0: push(get_param(vmg_ 0)); continue;
         ...
         case OPC_GETARGN1: push(get_param(vmg_ 1)); continue;

         case OPC_GETLCLN0: push(get_local(vmg_ 0)); continue;
         ...
         case OPC_RETNIL:
            ...
            /* return */
            if((p = do_return(vmg0_)) == 0) goto exit_loop;
            continue;
         ...
         case OPC_GETARGN2: push(get_param(vmg_ 2)); continue;
         ...
         case OPC_INDEX:
            ...
            if(apply_index(vmg_ valp, valp, get(0))) {
               ...
               pop_left_op(vmg_ & val);
               p = op_overload(vmg_ p - entry_ptr_native_,
                               -1,
                               &val,
                               G_predef->operator_idx,
                               1,
         ...
         case OPC_GETLCLN2: push(get_local(vmg_ 2)); continue;
         ...
         case OPC_GETLCLN1: push(get_local(vmg_ 1)); continue;

         case OPC_GETARGN3: push(get_param(vmg_ 3)); continue;

         case OPC_GETLCLN3: push(get_local(vmg_ 3)); continue;
         ...
         case OPC_ITERNEXT:
            ...
            if(valp->typ == VM_OBJ
               && vm_objp(vmg_ valp->val.obj)
                      ->iter_next(vmg_ valp->val.obj, &val)) {
         ...
         case OPC_GETLCLN4: push(get_local(vmg_ 4)); continue;
         ...
         case OPC_PUSHPARLST: {
            ...
            obj = CVmObjList::create_from_params(vmg_ cnt,
                                                 get_cur_argc(vmg0_) - cnt);
         ...
         case OPC_MAKELSTPAR: makelstpar(vmg0_); continue;
         ...
         case OPC_MOD:
            ...
            if(valp->typ == VM_INT && valp2->typ == VM_INT) {
               /* check for division by zero */
               if(valp->val.intval == 0) err_throw(VMERR_DIVIDE_BY_ZERO);
                ...
                */
               valp2->val.intval
                   = os_remainder_long(valp2->val.intval, valp->val.intval);
         ...
         case OPC_ASHR:
            ...
            if(valp->typ == VM_INT && valp2->typ == VM_INT) {
               /* compute the result */
               valp2->val.intval = t3_ashr(valp2->val.intval, valp->val.intval);
         ...
         case OPC_LSHR:
            ...
            if(valp->typ == VM_INT && valp2->typ == VM_INT) {
               /* compute the result */
               valp2->val.intval = t3_lshr(valp2->val.intval, valp->val.intval);
         ...
         case OPC_NOT:
            ...
            default: err_throw(VMERR_NO_LOG_CONV);
            }
            continue;
         ...
         case OPC_VARARGC: {
            ...
            switch(opc) {
            case OPC_CALL: goto do_opc_call;

            case OPC_PTRCALL: goto do_opc_ptrcall;

            case OPC_CALLPROP: goto do_opc_callprop;

            case OPC_PTRCALLPROP: goto do_opc_ptrcallprop;

            case OPC_CALLPROPSELF: goto do_opc_callpropself;

            case OPC_PTRCALLPROPSELF: goto do_opc_ptrcallpropself;

            case OPC_OBJCALLPROP: goto do_opc_objcallprop;

            case OPC_CALLPROPLCL1: goto do_opc_callproplcl1;

            case OPC_CALLPROPR0: goto do_opc_callpropr0;

            case OPC_INHERIT: goto do_opc_inherit;

            case OPC_PTRINHERIT: goto do_opc_ptrinherit;

            case OPC_EXPINHERIT: goto do_opc_expinherit;

            case OPC_PTREXPINHERIT: goto do_opc_ptrexpinherit;

            case OPC_DELEGATE: goto do_opc_delegate;

            case OPC_PTRDELEGATE: goto do_opc_ptrdelegate;

            case OPC_BUILTIN_A: goto do_opc_builtin_a;

            case OPC_BUILTIN_B: goto do_opc_builtin_b;

            case OPC_BUILTIN_C: goto do_opc_builtin_c;

            case OPC_BUILTIN_D: goto do_opc_builtin_d;

            case OPC_BUILTIN1: goto do_opc_builtin1;

            case OPC_BUILTIN2: goto do_opc_builtin2;

            case OPC_NEW1: goto do_opc_new1;

            case OPC_TRNEW1: goto do_opc_trnew1;

            case OPC_NEW2: goto do_opc_new2;

            case OPC_TRNEW2: goto do_opc_trnew2;

            default: err_throw(VMERR_INVALID_OPCODE_MOD); continue;
            }
         }
            continue;
         ...
         do_opc_ptrcall:
             ...
             */
            if(val.typ == VM_PROP && get_self_check(vmg0_) != VM_INVALID_OBJ)
         ...
         do_opc_ptrcallprop:
            ...
            /* evaluate the property */
            p = propev.get_prop(vmg_ p - entry_ptr_native_, val.val.prop, argc);
            continue;
         ...
         case OPC_GETLCLN5: pushval(vmg_ get_local(vmg_ 5)); continue;
         ...
         case OPC_SETSELF:
            ...
            if(val.typ == VM_NIL) val.val.obj = VM_INVALID_OBJ;
         ...
         case OPC_LOADCTX: {
            ...
            set_method_ctx(vmg_ vmb_get_dh_obj(lstp + VMB_LEN),
                           vmb_get_dh_prop(lstp + VMB_LEN + VMB_DATAHOLDER),
                           vmb_get_dh_obj(lstp + VMB_LEN + 2 * VMB_DATAHOLDER),
                           vmb_get_dh_obj(lstp + VMB_LEN + 3 * VMB_DATAHOLDER));
         ...
         case OPC_LRET:
            ...
            if(valp->typ != VM_INT) err_throw(VMERR_INT_VAL_REQD);
         ...
         case OPC_SAY: {
            ...
            /* display it */
            p = disp_dstring(
                vmg_ ofs, p - entry_ptr_native_, get_self_check(vmg0_));
         }
            continue;
         ...
         do_opc_expinherit:
            ...
            p = get_prop(
                vmg_ p - entry_ptr_native_, &val, prop, &val2, argc, 0);
            continue;
         ...
         case OPC_GETPROPDATA:
             ...
             */
            if(propev.self.typ != VM_OBJ) err_throw(VMERR_BAD_SPEC_EVAL);
         ...
         case OPC_SETDBARG:
         case OPC_GETPROPDATA:
         case OPC_PTRGETPROPDATA: err_throw(VMERR_NO_DEBUGGER); continue;
   ...
   err_catch(err)
   {
      ...
      err_try
      {
         ...
         if(err->get_error_code() == VMERR_UNHANDLED_EXC) {
            ...
            for(i = err->get_param_count(); i > 0;) {
               ...
               case ERR_TYPE_TEXTCHAR:
                  ...
                  push_obj(vmg_ CVmObjString::create(
                      vmg_ FALSE,
            ...
            if(G_predef->rterr != VM_INVALID_OBJ) {
               ...
               vm_objp(vmg_ G_predef->rterr)
                   ->create_instance(
                       vmg_ G_predef->rterr, 0, err->get_param_count() + 1);

               /* get the object from R0 */
               if(r0_.typ != VM_OBJ) err_throw(VMERR_EXCEPTION_OBJ_REQD);
               obj = r0_.val.obj;
            } else {
               /*
            ...
            if(G_predef->rterrmsg_prop != VM_INVALID_PROP) {
               ...
               msg = err_get_msg(
                   vm_messages, vm_message_count, err->get_error_code(), FALSE);
               ...
               str_obj = CVmObjString::create(vmg_ FALSE, buf, strlen(buf));
               ...
               val.set_obj(str_obj);
               vm_objp(vmg_ obj)->set_prop(
                   vmg_ G_undo, obj, G_predef->rterrmsg_prop, &val);
          ...
          */
         if(released_reserve) recover_reserve();
   ...
   if(unhandled_exc != VM_INVALID_OBJ) {
      ...
      err_throw_a(VMERR_UNHANDLED_EXC,
                  1,
                  ERR_TYPE_ULONG,
                  (unsigned long) unhandled_exc);
   }

   /* if we're not done, go back and resume execution */
   if(!done) goto resume_execution;
...
void CVmRun::validate_bifptr(VMG0_)
{
   ...
   if(!G_bif_table->validate_entry(bifp->val.bifptr.set_idx,
                                   bifp->val.bifptr.func_idx)) {
      /* invalid entry - throw an error */
      char buf[20];
      sprintf(buf, "#%d", bifp->val.bifptr.set_idx);
      err_throw_a(VMERR_UNAVAIL_INTRINSIC,
                  2,
                  ERR_TYPE_INT,
                  bifp->val.bifptr.func_idx,
                  ERR_TYPE_CHAR,
 ...
 */
void CVmRun::get_local_from_frame(VMG_ vm_val_t* val,
                                  vm_val_t* fp,
                                  const CVmDbgFrameSymPtr* symp)
{
   get_local_from_frame(vmg_ val,
                        fp,
                        symp->get_var_num(),
                        symp->is_param(),
                        symp->is_ctx_local(),
                        symp->get_ctx_arr_idx());
}
 ...
 */
void CVmRun::set_local_in_frame(VMG_ const vm_val_t* val,
                                vm_val_t* fp,
                                const CVmDbgFrameSymPtr* symp)
{
   set_local_in_frame(vmg_ val,
                      fp,
                      symp->get_var_num(),
                      symp->is_param(),
                      symp->is_ctx_local(),
 ...
 */
const uchar* CVmRun::get_named_args_from_frame(VMG_ vm_val_t* fp,
                                               vm_val_t** arg0)
{
   /* get the return address */
   pool_ofs_t ofs  = G_interpreter->get_return_ofs_from_frame(vmg_ fp);
   const uchar* ep = G_interpreter->get_enclosing_entry_ptr_from_frame(vmg_ fp);
 ...
 *   on the stack.
 */
void CVmRun::throw_new_class(VMG_ vm_obj_id_t cls,
                             uint argc,
                             const char* fallback_msg)
{
   ...
   if(cls != VM_INVALID_OBJ) {
      ...
      if(r0_.typ == VM_OBJ) {
          ...
          */
         err_throw_a(
             VMERR_UNHANDLED_EXC, 1, ERR_TYPE_ULONG, (unsigned long) exc_obj);
      } else
...
const uchar* CVmRun::do_throw(VMG_ const uchar* pc, vm_obj_id_t exception_obj)
{
   ...
   for(;;) {
      ...
      if(func_start != 0 && tab.set(func_start)) {
         ...
         for(i = 0, cnt = tab.get_count(); i < cnt; ++i, entry.inc(vmg0_)) {
            /*
             ...
             */
            if(ofs >= entry.get_start_ofs() && ofs <= entry.get_end_ofs()
 ...
 */
const uchar* CVmRun::call_func_ptr(VMG_ const vm_val_t* funcptr,
                                   uint argc,
 ...
 */
const uchar* CVmRun::call_func_ptr_fr(VMG_ const vm_val_t* funcptr,
                                      uint argc,
                                      const vm_rcdesc* recurse_ctx,
                                      uint caller_ofs)
{
   ...
   case VM_OBJ:
      ...
      if((io = vm_objp(vmg_ funcptr->val.obj))->get_invoker(vmg_ & invoker)) {
         ...
         case VM_CODEPTR: invoke_addr = invoker.val.ptr; break;

         default: invoke_addr = 0; break;
         }
         ...
         if(invoke_addr != 0) {
            ...
            return do_call(
                vmg_ caller_ofs, (const uchar*) invoke_addr, argc, recurse_ctx);
         }
      }
   ...
   case VM_BIFPTR:
      ...
      discard(5);
      call_bif(
          vmg_ funcptr->val.bifptr.set_idx, funcptr->val.bifptr.func_idx, argc);
 ...
 */
const uchar*
CVmRun::do_call_func_nr(VMG_ uint caller_ofs, pool_ofs_t target_ofs, uint argc)
{
   ...
   if(!hdr_ptr.argc_ok(argc)) err_throw(VMERR_WRONG_NUM_OF_ARGS);
   ...
   for(i = lcl_cnt; i != 0; --i) (fp++)->set_nil();

   /* create and activate the new function's profiler frame */
   VM_IF_PROFILER(if(profiling_) prof_enter(vmg_ target_ofs_ptr));
                             ...
                             const vm_rcdesc* recurse_ctx)
{
   ...
   if(!hdr_ptr.argc_ok(argc)) {
      ...
      if(recurse_ctx != 0) {
         ...
         err_throw_a(VMERR_WRONG_NUM_OF_ARGS_CALLING,
                     1,
                     ERR_TYPE_CHAR,
                     recurse_ctx->name);
      } else {
   ...
   /* create and activate the new function's profiler frame */
   VM_IF_PROFILER(if(profiling_) prof_enter(vmg_ target_ptr));
...
const uchar* CVmRun::do_return(VMG0_)
{
   ...
   if(fro->typ == VM_OBJ && CVmObjFrameRef::is_frameref_obj(vmg_ fro->val.obj))
   ...
   VM_IF_PROFILER(if(profiling_) prof_leave());
 ...
 */
void CVmRun::set_prop(VMG_ vm_obj_id_t obj,
                      vm_prop_id_t prop,
                      const vm_val_t* new_val)
{
   /* set the property */
 ...
 */
const uchar* CVmRun::op_overload(VMG_ uint caller_ofs,
                                 int asi_lcl,
                                 const vm_val_t* obj,
                                 vm_prop_id_t prop,
                                 uint argc,
                                 int err)
{
    ...
    */
   if(prop == VM_INVALID_PROP) err_throw(err);
   ...
   case VM_LIST:
      ...
      if(get_prop_no_eval(
             vmg_ & obj, prop, &argc, &srcobj, &val, &obj, &new_self)) {
          ...
          */
         if(val.typ == VM_CODEOFS || val.typ == VM_DSTRING
            || val.typ == VM_OBJX) {
            /*
         ...
         /* evaluate the property */
         const uchar* ret = eval_prop_val(vmg_ TRUE,
                                          caller_ofs,
                                          &val,
                                          obj->val.obj,
                                          prop,
                              ...
                              const vm_rcdesc* rc)
{
   ...
   /* find the property without evaluating it */
   found = get_prop_no_eval(
       vmg_ & target_obj, target_prop, &argc, &srcobj, &val, &self, &new_self);
   ...
   return eval_prop_val(vmg_ found,
                        caller_ofs,
                        &val,
                        self->val.obj,
                        target_prop,
                      ...
                      const vm_rcdesc* rc)
{
   ...
   if(prop == VM_INVALID_PROP) return;
                                    ...
                                    vm_val_t* new_self)
{
   ...
   case VM_LIST:
      ...
      found = CVmObjList::const_get_prop(
          vmg_ val, *target_obj, target_ptr, target_prop, srcobj, argc);
   ...
   case VM_OBJ:
      ...
      if((*self)->typ != VM_OBJ) err_throw(VMERR_OBJ_VAL_REQD);
      break;
 ...
 */
const uchar* CVmRun::eval_prop_val(VMG_ int found,
                                   uint caller_ofs,
                                   const vm_val_t* val,
                                   vm_obj_id_t self,
                                   vm_prop_id_t target_prop,
                                   ...
                                   const vm_rcdesc* rc)
{
   ...
   if(found) {
      ...
      case VM_BIFPTRX:
         /* Execute-on-eval built-in function.  Call the function. */
         call_bif(vmg_ val->val.bifptr.set_idx, val->val.bifptr.func_idx, argc);
 ...
 */
const uchar*
CVmRun::inh_prop(VMG_ uint caller_ofs, vm_prop_id_t prop, uint argc)
{
   ...
   found = vm_objp(vmg_ self)->inh_prop(vmg_ prop,
                                        &val,
                                        self,
   ...
   if(!found && G_predef->prop_not_defined_prop != VM_INVALID_PROP) {
       ...
       */
      found = vm_objp(vmg_ self)->inh_prop(vmg_ G_predef->prop_not_defined_prop,
                                           &val,
                                           self,
                                           orig_target_obj.val.obj,
                                           defining_obj,
 ...
 */
const uchar*
CVmRun::disp_dstring(VMG_ pool_ofs_t ofs, uint caller_ofs, vm_obj_id_t self)
{
   /* push the string */
...
const uchar* CVmRun::disp_string_val(VMG_ uint caller_ofs, vm_obj_id_t self)
{
   ...
   if(say_method_ != VM_INVALID_PROP && self != VM_INVALID_OBJ) {
       ...
       *   method, invoke it
       */
      if(vm_objp(vmg_ self)->get_prop(vmg_ say_method_, &val, self, &src_obj, 0)
         && (val.typ == VM_CODEOFS || val.typ == VM_OBJX
             || val.typ == VM_BIFPTRX)) {
         ...
         return eval_prop_val(vmg_ TRUE,
                              caller_ofs,
                              &val,
                              self,
                              say_method_,
...
void CVmRun::set_say_func(VMG_ const vm_val_t* val)
{
    ...
    *   allocate one now
    */
   if(say_func_ == 0) say_func_ = G_obj_table->create_global_var();
...
vm_val_t* CVmRun::get_fp_at_level(VMG_ int level) VM_REG_CONST
{
    ...
    */
   if(fp == 0 || (rc != 0 && rc->has_return_addr())) err_throw(VMERR_BAD_FRAME);
   ...
   if(exc->get_error_code() == VMERR_UNHANDLED_EXC) {
      ...
      if(msg != 0) {
         ...
         if(add_unh_prefix) {
            ...
            tmpmsg = err_get_msg(vm_messages,
                                 vm_message_count,
         ...
         tmpmsg = err_get_msg(
             vm_messages, vm_message_count, VMERR_UNHANDLED_EXC, FALSE);
         err_format_msg(buf, buflen, tmpmsg, &tmpexc);
      }
   } else {
      /*
       ...
       */
      msg = err_get_msg(
          vm_messages, vm_message_count, exc->get_error_code(), FALSE);

      /* if that failed, just show the error number */
      if(msg == 0) {
         /* no message - just show the error code */
         tmpmsg = err_get_msg(
             vm_messages, vm_message_count, VMERR_VM_EXC_CODE, FALSE);
         ...
         tmpmsg = err_get_msg(
             vm_messages, vm_message_count, VMERR_VM_EXC_PARAM, FALSE);
...
const char* CVmRun::get_exc_message(VMG_ vm_obj_id_t exc_obj, size_t* msg_len)
{
   ...
   if(exc_obj == VM_INVALID_OBJ) return 0;
   ...
   argc = 0;
   if(G_predef->rterrmsg_prop == VM_INVALID_PROP
      || (!vm_objp(vmg_ exc_obj)
               ->get_prop(vmg_ G_predef->rterrmsg_prop,
                          &val,
                          exc_obj,
                          &src_obj,
                          &argc)))
                           ...
                           const uchar** stm_end)
{
   ...
   *stm_start = *stm_end = 0;
    ...
    *   boundaries
    */
   if(!func_ptr->set_dbg_ptr(&dbg_ptr) || dbg_ptr.get_line_count(vmg0_) == 0) {
   ...
   while(lo <= hi) {
      ...
      if(method_ofs >= end_ofs) {
         ...
         *stm_end   = func_ptr->get() + end_ofs;
         ...
         if(caller_line_ptr != 0) caller_line_ptr->copy_from(&line_ptr);
...
{
 public:
   CVmHashEntryProfiler(const char* str, size_t len, const vm_profiler_rec* rec)
...
void CVmRun::end_profiling()
{
   ...
   while(prof_stack_idx_ != 0) prof_leave();
}
...
struct vmrun_prof_enum
{
   ...
   void (*cb)(void*, const char*, unsigned long, unsigned long, unsigned long);
   void* cb_ctx;
};
 ...
 */
void CVmRun::get_profiling_data(VMG_ void (*cb)(void*,
                                                const char*,
                                                unsigned long,
                                                unsigned long,
                                                unsigned long),
 ...
 */
void CVmRun::prof_calc_elapsed(vm_prof_time* diff,
                               const vm_prof_time* a,
