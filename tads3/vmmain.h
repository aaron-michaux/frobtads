...
#ifdef WASMTADS
/*
 * Returns the same values as `vm_get_game_type`, but for an already
 * opened `FILE*`.
 */
...
class CVmMainClientIfc
{
 public:
    ...
    */
   virtual class CVmConsoleMain* create_console(struct vm_globals* globals) = 0;
...
class CVmMainClientIfcStdio : public CVmMainClientIfc
{
 public:
   ...
   virtual void client_init(struct vm_globals*,
                            const char*,
                            int,
                            const char*,
                            const char*,
